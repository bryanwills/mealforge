...
export default function ExplorePage() {
  ...
  const fetchRecipes = useCallback(async (searchParams: Record<string, string> = {}) => {
    ...
    try {
      ...
      let savedExternalIds: string[] = []
      if (savedResponse.ok) {
        const savedData = await savedResponse.json()
        savedExternalIds = savedData.savedRecipes
          .filter((recipe: { externalId?: string }) => recipe.externalId)
          .map((recipe: { externalId: string }) => recipe.externalId)
      }

      // Update recipes with saved state
      const recipesWithSavedState = data.results.map((recipe: Recipe) => ({
        ...recipe,
        isSaved: savedExternalIds.includes(recipe.id)
      }))
